...
| `entrypoint` | [Command](common-types.md#command) | no |  | Optional override for the image `ENTRYPOINT`, the executable the container runs. See [`Command`] for the accepted forms. Independent from `command`: setting this leaves the image `CMD` in place, which means it is appended as arguments to the new entrypoint. An empty list is rejected; omit the field to keep the image entrypoint. |
