...
- `lightshuttle_spec::Argument`, an `entrypoint`/`command` element that is either a `Literal` value copied verbatim or a `Secret` naming an environment key to look up instead. `lightshuttle_runtime` resolves it against the container's own `env` immediately before `docker create`, and every export target renders a `Secret` in its own reference syntax rather than the value it names.
...
- **Breaking change:** a Compose secret variable is now named `<RESOURCE>_<KEY>` instead of the bare `<KEY>` (see the Security entry below for why). `lightshuttle export compose` interpolating from one project-wide `.env` meant two resources sharing a key, such as two `postgres` databases both declaring `POSTGRES_PASSWORD`, silently received the same value. Every `.env` file, secret store or pipeline populating one for an existing exported stack must rename its variables to the resource-qualified form, for example `POSTGRES_PASSWORD` becomes `DB_POSTGRES_PASSWORD` for a resource named `db`; re-run the export and read the generated `docker-compose.yml` to confirm the exact names. `ContainerSpec::entrypoint` and `ContainerSpec::command` (`lightshuttle-spec`) also change type, from `Option<Vec<String>>` to `Option<Vec<Argument>>`; the struct is `#[non_exhaustive]`, but this is a breaking change for any code that read or built either field directly.
...
### Fixed
- `lightshuttle export compose` no longer emits a `depends_on` entry naming a resource the same export excludes (#283). A resource disabled through `export.compose.resources.<name>.enabled: false` left every dependent service pointing at a name the file never defines, and `docker compose config` rejected the result (`depends on undefined service`). Such an export is now refused, naming every offending pair of exported service and disabled dependency at once, so the manifest is fixed in one pass by disabling the dependent services too or by dropping the dependency. Kubernetes and Helm emit no `depends_on` and are unaffected.
- Symbolic links in a `dockerfile` build context are no longer dropped (#287). The archive sent to the daemon only held directories and regular files, so `COPY` of a linked file or directory failed with `not found` under LightShuttle while `docker build` succeeded. Links are now archived as links with their target unchanged and are never followed, as `docker build` does: a link to a file or directory inside the context works as a `COPY` source, and a link whose target is outside the context, absolute or missing is kept as a link that the daemon cannot resolve, so no host file is reached through it. `.dockerignore` applies to link paths.
- An export no longer hands `${...}` references to the deployment target verbatim (#308). `lightshuttle export compose` wrote a file `docker compose config` refused (`invalid interpolation format for services.api.image`), `lightshuttle export helm` failed outright on an image such as `example/api:${env.TAG:-1.0}` (`invalid image tag '-1.0}'`), and `lightshuttle export kubernetes` wrote references verbatim without reporting anything, producing a manifest no deploy step would ever substitute. References are now interpreted once per target: a `${resources.*.*}` reference is resolved against the hostname the target reaches the service through (the raw service name for Compose, the DNS-sanitised name for Kubernetes and Helm), and a `${env.*}` reference is rendered in the target's own syntax. Compose emits `${NAME}` and `${NAME:-default}` and escapes a literal `$` to `$$`; Helm publishes each variable under a top-level `variables:` key of `values.yaml`, overridable with `--set variables.NAME=value`; plain Kubernetes freezes every default in place and refuses an export whose variables have none, naming all of them at once. A variable name must match `[A-Za-z_][A-Za-z0-9_]*`, which Compose interpolation and Helm value keys both require.
- A Helm chart whose image reference holds a `${env.*}` variable is now published whole under `image.reference` rather than split into `repository` and `tag` (#308). The split cannot tell a missing tag from a tag that is still a placeholder, and guessing would deploy `latest` without saying so. Such a service loses its separate `repository` and `tag` knob in `values.yaml`.
- `lightshuttle export compose` now emits `working_dir`. The field had no place in the generated service, so a manifest working directory was dropped in silence and the exported stack ran in the image's default directory.
...
- Every interpolatable field is now resolved before a resource is started, not just environment values and the command (#276). A `${env.*}` or `${resources.*.*}` reference in an image, entrypoint, working directory, volume, healthcheck or Dockerfile build input previously reached Docker verbatim, so a manifest such as `image: "app:${env.TAG}"` passed `validate` but ran the literal reference. A single canonical field walk now drives both reference scanning (validation, implicit dependency discovery) and substitution, so the set of fields advertised as interpolatable can no longer drift from the set actually resolved, and `entrypoint`, previously omitted from both, is covered. Interpolation happens before the resource is lowered, so the image, volume and healthcheck parsers only ever see fully resolved values. As a consequence, spec-build failures (an invalid image reference, port or volume mapping) now surface when the resource is started rather than when the plan is built; both occur during `lightshuttle up`. Export still emits unresolved `${env.*}` references verbatim; rewriting them as deployment placeholders is tracked separately.
...
- A foreground `lightshuttle restart <resource>` no longer hangs until its timeout when the restart finishes quickly (#277). The CLI sent the restart POST first and only then opened `/ws/events`, but lifecycle events are broadcast without replay, so a `resource_healthy` or `resource_failed` emitted before the subscription was lost and the command waited the full 90 seconds for an event that had already passed. The CLI now subscribes to the event stream before triggering the restart, so the terminal event can never land in a window with no subscriber. A duplicate restart already in flight is now surfaced as a clear error rather than an "unexpected control plane response", matching the `409 Conflict` introduced in #280. `--detach` still returns as soon as the restart is accepted, without opening the event stream.
...
### Security
- A managed `redis` resource's password no longer reaches an exported Compose, Kubernetes or Helm artifact in clear text. Redaction only ever looked at environment key names, but `spec_redis` wrote the password straight into the container's `command` as `--requirepass <value>`, a channel no export target inspected. The password now travels through a `REDIS_PASSWORD` environment key, classified as a secret like any other, and the command carries a reference to that key instead: `$(REDIS_PASSWORD)` on Kubernetes and Helm, `${<RESOURCE>_REDIS_PASSWORD}` on Compose. A `redis` resource with no password still gets no `--requirepass` flag and no `REDIS_PASSWORD` key. The value still becomes a plain process argument when the container starts, so it remains readable by anyone who can see that process (a `ps` inside the container or on the machine running it, `docker inspect`, the Kubernetes node); the exported files, the pod spec served by the cluster API and the repository stay clean. Two secret keys whose Compose variable names collide, across resources or within one, are refused at export.
- A credential now travels with the value rather than with the key name (#308). An environment key that receives a `${resources.<name>.password}` or `${resources.<name>.url}` reference is classified as a secret on every export target, whatever the key is called: Compose writes `${KEY}`, Kubernetes and Helm write `'***'` in their `Secret`. A key such as `DATABASE_URL` matches none of the name markers below, so before this change the database password reached the exported artifact in clear text as soon as the reference was resolved. The same reference outside an environment value, in an image, an argv element, a working directory or a healthcheck, has nowhere safe to hide and is now refused with the resource, the field and the reference named.
- A literal `{{` reaching a Helm value rendered through `tpl` is escaped to `{{ "{{" }}`, which Go renders back to a literal `{{`. `values.yaml` is data and Helm does not template it, so a value is passed through `tpl` only for a service that actually carries a deployment variable: a chart that gains nothing from it does not pay its injection surface either. The Helm `workingDir` field, previously written unquoted and unescaped unlike the argv arrays next to it, goes through the same escape.
